...
class LSHCache:
    ...
    def __init__(self, n=100, b=20, r=5, max_shingle=3):
        ...
        # check it
        assert self._b * self._r == self._n, 'Minhash bands/rows/length mismatch: _b*_r != _n, _b=%d, _r=%d, _n=%d' % (
        self._b, self._r, self._n)
        assert self._max_shingle > 0, '_max_shingle must be greater than 0.  Current _max_shingle=%d' % (
            self._max_shingle)
    ...
    def insert_batch(self, doc_tuples):
        ...
        dup_buckets = self.get_dup_buckets(doc)
